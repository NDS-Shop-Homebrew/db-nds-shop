...
export default function GameList() {
  ...
  const normalizeGame = (g: RawGameItem): Game => ({
    id: g.id || g.fileName,
    slug: g.slug || g.fileName || g.id,
    fileName: g.fileName || g.slug || g.id || "",
    title: g.title || "",
    author: g.author || "",
    version: g.version || "",
    ...
    icon: resolveAssetUrl(g.iconUrl || g.icon),
    updated: g.updated || g.updatedAt || "",
    screenshots: (g.screenshots || []).map((s: RawScreenshot) => ({
      description: s.description || "",
      url: resolveAssetUrl(s.url),
    })),
  });

  useEffect(() => {
    fetch(`${API_BASE}/api/v1/games`)
      .then((res) => (res.ok ? (res.json() as Promise<GamesApiResponse | RawGameItem[]>) : { games: [] }))
      .then((data) => {
        const list = (Array.isArray(data) ? data : data.games || []).map(normalizeGame);
        setGames(list);
      })
      .catch((err: unknown) => console.error("Failed to load games:", err))
  ...
  const catLabel = (c: string) =>
    c === "game"
      ? t("gameList.cat.game")
      : c === "homebrew"
      ? t("gameList.cat.homebrew")
      : c === "emulator"
      ? t("gameList.cat.emulator")
      : c;
  ...
  const filtered = useMemo(() => {
    ...
    if (filterCategory !== "all")
      result = result.filter((g) => g.categories?.includes(filterCategory));
    if (filterLetter !== "all")
      result = result.filter(
        (g) => (g.title[0] || "").toUpperCase() === filterLetter
      );
    if (regionFilter)
      result = result.filter((g) => g.version?.includes(regionFilter));
    ...
    result.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "updated":
          cmp =
            (new Date(b.updated).getTime() || 0) - (new Date(a.updated).getTime() || 0);
          break;
        case "popular":
          cmp = (counts[b.fileName] || 0) - (counts[a.fileName] || 0);
          break;
        default:
    ...
    >
      <span
        className={`inline-flex items-center gap-1 transition-colors ${
          sortBy === field ? "text-primary font-semibold" : "hover:text-foreground"
        }`}
      >
              ...
              <SelectContent>
                {categories.map((c) => (
                  <SelectItem key={c} value={c}>
                    {c === "all" ? t("gameList.allCategories") : catLabel(c)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
                  ...
                  {pageItems.map((game) => (
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground whitespace-nowrap">
                        {counts[game.fileName] != null &&
                          counts[game.fileName] > 0 && (
                      ...
                      <TableCell className="text-muted-foreground/60 text-xs whitespace-nowrap">
                        {game.updated ? new Date(game.updated).toLocaleDateString(i18n.language) : "—"}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
