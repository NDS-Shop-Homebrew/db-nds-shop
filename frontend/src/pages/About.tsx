...
const getActivityPrefix = (activity: DiscordActivity, t: (key: string) => string) => {
...
export default function About() {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const teamRes = await fetch(`${API_BASE}/api/v1/team`);
        const team = (await teamRes.json()) as TeamResponse;
        ...
        const users = await Promise.all(
          teamMembers.map(async (m) => {
            const res = await fetch(`${API_BASE}/api/v1/discord-user/${m.id}`);
  ...
  useEffect(() => {
    const fetchPresence = async () => {
      try {
        const teamRes = await fetch(`${API_BASE}/api/v1/team`);
        const team = (await teamRes.json()) as TeamResponse;
        const ids: string[] = (team.members || []).map((m) => m.id);
        const pres = await Promise.all(
          ids.map(async (id) => {
            const res = await fetch(`${API_BASE}/api/v1/discord-presence/${id}`);
            if (!res.ok) return null;
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/v1/discord-guild`)
      .then((r) => (r.ok ? r.json() : null))
      .then((g: DiscordGuild | null) => {
                ...
                <div className="flex items-center justify-center sm:justify-start gap-4 mt-3 text-sm text-muted-foreground">
                  {typeof guild.memberCount === "number" && (
                    <span className="flex items-center gap-1.5">
                      ...
                      {t("about.discord_members")}
                    </span>
                  )}
                  {typeof guild.presenceCount === "number" && (
                    <span className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-[#5865F2]" />
                      {guild.presenceCount.toLocaleString()}{" "}
                      {t("about.discord_online")}
                    </span>
                  )}
                </div>
              </div>
                    ...
                    {userPresence?.activities?.length ? (
                      <div className="mt-4 flex flex-col gap-2 w-full">
                        {userPresence.activities.map((activity, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 bg-muted p-3 rounded-xl shadow-sm"
                          >
